...
def apicall(api_url:str, timeout:int=_DEFAULT_API_TIMEOUT) -> tuple[bool,dict|str]:
...
def get_site_name(addr:str) -> str:
    ''' returns the name of the site, on error return empty '''
    success,result = apicall( addr + "/name" )
    return result["name"] if success and "name" in result.keys() else ""
...
def warp_connect(addr:str) -> bool:
    ...
    return success and result["status"] == "success"
